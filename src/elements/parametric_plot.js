...
const parametricPlotInterface = constructInterface({
  interface: {
    ...
    varName: { description: "The name of the varying variable", typecheck: "VariableName" },

    range: { description: "Range of the varying variable to plot", aliases: ['t'] },

    function: { setAs: "user", description: "The function R -> R^2 of the plot", aliases: ['f'] },

    samples: { description: "The number of samples to plot", typecheck: { type: "integer", min: 10, max: 1e6 } }
  }, internal: {
    // Pen used to draw the parametric plot
    pen: { type: "Pen", computed: "user", default: DefaultStyles.Pen, compose: true },
...
export class ParametricPlot2D extends Element {
  ...
  compileFunction () {
    ...
    if (props.hasChanged("function")) {
      ...
      if (node.type !== "vec2")
        throw new Error(`Expected expression with return type of vec2, got expression with return type of ${node.type}`)
  ...
  computePoints () {
    ...
    let rangeStart = range[0], rangeEnd = range[1], rangeLen = rangeEnd - rangeStart
    ...
    this.internal.renderInfo = { instructions: { type: "polyline", vertices: pts, pen } }
  }
