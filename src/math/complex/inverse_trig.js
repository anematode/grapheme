...
export const Arcsin = z =>
  Multiply(
    Complex.I.scale(-1), // -i
    Ln(
      Add(
        Multiply(Complex.I, z), // i * z
        Sqrt(Subtract(Complex.One, Multiply(z, z)))
      )
    )
  ) // sqrt(1 - z^2

// arccos(z) = pi/2 + i * ln(i * z + sqrt(1 - z^2))
export const Arccos = z =>
  Add(
    new Complex(Math.PI / 2), // pi / 2
    Multiply(
      Complex.I,
      Ln(
        Add(
          Multiply(Complex.I, z), // i * ln(iz
          Sqrt(Subtract(Complex.One, Multiply(z, z)))
        )
      )
    )
...
export const Arctan = z =>
  Multiply(
    Complex.I.scale(1 / 2), // i / 2
    Ln(Divide(Add(Complex.I, z), Subtract(Complex.I, z)))
  ) // ln( (i+z) / (1-z) )
...
export const ArcsecBranched = (z, branch = 0) =>
  ArccosBranched(Divide(Complex.One, z), branch)

export const ArccscBranched = (z, branch = 0) =>
  ArcsinBranched(Divide(Complex.One, z), branch)
