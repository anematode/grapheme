...
function Polygamma (m, z) {
  if (m < 0) return new Complex(NaN, NaN)
  if (m % 1 !== 0) return new Complex(NaN, NaN)

  if (m === 0) return Digamma(z)
  else if (m === 1) return Trigamma(z)

  let sign = m % 2 === 0 ? -1 : 1
  let numPoly = getPolygammaNumeratorPolynomial(m)

  if (z < 0.5) {
    if (z % 1 === 0) return new Complex(Infinity)
    ...
    return Multiply(
      ...
      Divide(
        numPoly
          .evaluateComplex(Cos(z.scale(Math.PI)))
          .scale(Math.pow(Math.PI, m + 1)),
        PowN(Sin(z.scale(Math.PI)), m + 1) +
          sign * Polygamma(m, Subtract(Complex.One, z))
      )
    )
  } else if (z < 8) {
    // Recurrence relation
    ...
    return Add(
      Polygamma(m, z + 1),
      Divide(new Complex(sign * gamma(m + 1)), PowN(z, m + 1))
    )
  }
