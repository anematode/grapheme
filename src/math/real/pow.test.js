...
const bases = [-2, -1.121, -1, -141004001001.4, -24194.140198]

test('returns NaN for negative base with exponent with even denominator', () => {
  const angers = [151 / 2, 15 / 2, 429 / 2, 511 / 12, 1581 / 144, 50181 / 502]
  ...
  for (const neg of happyNegatives) {
    for (const base of bases) {
      for (const cow of getSurrounding(neg)) {
        expect(pow(base, cow), `Input was ${base} ^ ${cow}`).toBe(
          -Math.pow(-base, neg)
        )
      }
    }
...
test('Various other fractions correct', () => {
  ...
  const angers = [
    ...fractions.map(roundUp).map(roundUp),
    ...fractions.map(roundDown).map(roundDown)
  ]
