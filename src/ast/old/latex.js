...
function genFunctionLatex (functionName) {
  let fName =
    functionName[0] === '\\' ? functionName : `\\operatorname{${functionName}}`

  return (nodes, params = {}) => {
    if (nodes.length === 1) {
      if (
        optionalParentheses.includes(functionName) &&
        !needsParentheses(nodes[0])
      ) {
        return String.raw`${fName} ${nodes[0].latex(params)}`
      }
    }

    return String.raw`${fName}\left(${nodes
      .map(node => node.latex(params))
      .join(', ')}\right)`
  }
}
...
function genFunctionSubscriptLatex (functionName) {
  ...
  return (nodes, params = {}) =>
    String.raw`${fName}_{${nodes[0].latex(params)}}\left(${nodes
      .slice(1)
      .map(node => node.latex(params))
      .join(', ')}\right)`
}
...
function getVariableLatex (str) {
  ...
  components = components.map(str => {
    ...
    if (str[0] !== '\\' && str.length > 1) str = String.raw`\text{${str}}`
...
function polygammaLatex (nodes, params = {}) {
  return String.raw`\psi^{(${nodes[0].latex(params)})}\\left(${nodes[1].latex(
    params
  )}\\right)`
}
