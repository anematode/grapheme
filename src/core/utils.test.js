...
describe('deepEquals', () => {
  ...
  test('Objects', () => {
    ...
    expect(deepEquals({ x: { x: {} }, y: NaN }, { x: { x: {} }, y: NaN })).toBe(
      true
    )
  ...
  test('Typed arrays', () => {
    expect(
      deepEquals(new Int32Array([-1, 0, 1]), new Int32Array([-1, 0, 1]))
    ).toBe(true)
    expect(
      deepEquals(new Int32Array([-1, 0, 1]), new Int16Array([-1, 0, 1]))
    ).toBe(false)

    expect(
      deepEquals(new Float64Array([3, 4, 1]), new Float64Array([2, 0, 2]))
    ).toBe(false)
    expect(
      deepEquals(new Float64Array([3, 4, 1]), new Float64Array([3, 4, 1]))
    ).toBe(true)
    expect(
      deepEquals(
        new Float64Array([NaN, NaN, NaN, NaN]),
        new Float64Array([NaN, NaN, NaN, NaN])
      )
    ).toBe(true)
    expect(
      deepEquals(
        new Float64Array([NaN, NaN, NaN, -0]),
        new Float64Array([NaN, NaN, NaN, -0])
      )
    ).toBe(true)
    expect(
      deepEquals(
        new Float64Array([0, NaN, NaN, -0]),
        new Float64Array([0, NaN, NaN, -0])
      )
    ).toBe(true)
    expect(
      deepEquals(
        new Float64Array([0, -0, NaN, -0]),
        new Float64Array([0, 0, NaN, -0])
      )
    ).toBe(false)
    expect(deepEquals(new Float64Array([NaN]), new Float64Array([0]))).toBe(
      false
    )
    expect(deepEquals(new Float64Array([NaN]), new Float64Array([1]))).toBe(
      false
    )
    expect(deepEquals(new Float64Array([0]), new Float64Array([NaN]))).toBe(
      false
    )
    expect(deepEquals(new Float64Array([1]), new Float64Array([NaN]))).toBe(
      false
    )
  })
